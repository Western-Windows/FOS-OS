...
void free_user_mem(struct Env* e, uint32 virtual_address, uint32 size)
{
		...
		struct WorkingSetElement* pre_last = head;
		while (pre_last->prev_next_info.le_next != last_ws) {
			pre_last = pre_last->prev_next_info.le_next;
		}
		struct WorkingSetElement* temp = e->page_WS_list.lh_last;
		temp->prev_next_info.le_next = head;
		head->prev_next_info.le_prev = temp;
		e->page_WS_list.lh_first = last_ws;
		last_ws->prev_next_info.le_prev = NULL;
		e->page_WS_list.lh_last = pre_last;
		pre_last->prev_next_info.le_next = NULL;
}
