...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	//====================================================================//
	//THIS IS THE START OF ADDED TESTS//
	//====================================================================//
	cprintf("THIS IS THE START OF ADDED TESTS\n");

	//[1] realloc with same size (NOTHING CHANGES)
	cprintf("realloc with same size (NOTHING CHANGES)\n");
	is_correct = 1;
	{
		blockIndex = 1*allocCntPerSize + 1;
		old_size = allocSizes[1] - sizeOfMetaData;/*20 B*/
	...
	//[2] decrease size, next block free split,turn to block and merge with next (Coalesce)

		is_correct = 1;
		{
			blockIndex = 1;
			old_size = 3072 - sizeOfMetaData; /*4KB - sizeOfMetaData*/;
			new_size = old_size - 1*kilo ;
			expectedSize = ROUNDUP(new_size + sizeOfMetaData, 2);
			expectedVA = startVAs[blockIndex];
	...
	//[3] exactly same size of next block
	is_correct = 1;
				{
				blockIndex = 1;
				old_size = 2048 - sizeOfMetaData; /*4KB - sizeOfMetaData*/;
				new_size = 2048*2 -sizeOfMetaData  ;
				expectedSize = ROUNDUP(new_size + sizeOfMetaData, 2);
				expectedVA = startVAs[blockIndex];
...
//
	...
	//[4] decrease size, next block free split and merge (Coalesce)
	is_correct = 1;
			{
				blockIndex = 399;
				old_size = 28 - sizeOfMetaData; /*4KB - sizeOfMetaData*/;
				new_size = old_size - 4 ;
				expectedSize = ROUNDUP(new_size + sizeOfMetaData, 2);
				expectedVA = startVAs[blockIndex];
...
//			}
	...
	{
		blockIndex = 1200;
		old_size = 4096 - sizeOfMetaData; /*4KB - sizeOfMetaData*/;
		new_size = 4160 - sizeOfMetaData;
		expectedSize = ROUNDUP(new_size + sizeOfMetaData, 2);
		expectedVA = startVAs[blockIndex];

		va = realloc_block_FF(startVAs[199], new_size);
...
//				cprintf("%d %d",get_block_size(va),is_free_block(va));
	...
	cprintf("[UNSEEN] test realloc_block with FIRST FIT completed. Evaluation = %d%\n", unseeneval);
}
